resource "aws_db_event_subscription" "slack_notifications" {
  count     = length(var.actions)
  name      = "${var.prefix}-rds-event-subcription"
  sns_topic = var.actions[count.index]
  ...
  tags = merge(var.default_tags,
    {
      "target" : var.actions[count.index]
    }
  )
}
...
resource "aws_cloudwatch_metric_alarm" "rds_cpu_utilization_too_high" {
  alarm_name                = "${var.prefix}-rds-${var.rds_instance_id}-highCPUUtilization"
...
resource "aws_cloudwatch_metric_alarm" "cpu_credit_balance_too_low" {
  alarm_name                = "${var.prefix}-rds-${var.rds_instance_id}-lowCPUCreditBalance"
...
resource "aws_cloudwatch_metric_alarm" "disk_queue_depth_too_high" {
  alarm_name                = "${var.prefix}-rds-${var.rds_instance_id}-highDiskQueueDepth"
  comparison_operator       = "GreaterThanThreshold"
  evaluation_periods        = var.evaluation_period
...
resource "aws_cloudwatch_metric_alarm" "disk_free_storage_space_too_low" {
  alarm_name                = "${var.prefix}-rds-${var.rds_instance_id}-lowFreeStorageSpace"
  comparison_operator       = "LessThanThreshold"
  evaluation_periods        = var.evaluation_period
...
resource "aws_cloudwatch_metric_alarm" "disk_burst_balance_too_low" {
  alarm_name                = "${var.prefix}-rds-${var.rds_instance_id}-lowEBSBurstBalance"
  comparison_operator       = "LessThanThreshold"
  evaluation_periods        = var.evaluation_period
...
resource "aws_cloudwatch_metric_alarm" "memory_freeable_too_low" {
  alarm_name                = "${var.prefix}-rds-${var.rds_instance_id}-lowFreeableMemory"
  comparison_operator       = "LessThanThreshold"
  evaluation_periods        = var.evaluation_period
...
resource "aws_cloudwatch_metric_alarm" "memory_swap_usage_too_high" {
  alarm_name                = "${var.prefix}-rds-${var.rds_instance_id}-highSwapUsage"
  comparison_operator       = "GreaterThanThreshold"
  evaluation_periods        = var.evaluation_period
...
resource "aws_cloudwatch_metric_alarm" "connection_count_too_high" {
  alarm_name                = "${var.prefix}-rds-${var.rds_instance_id}-highConnectionCount"
  comparison_operator       = "GreaterThanThreshold"
  evaluation_periods        = var.evaluation_period
  ...
  statistic                 = "Average"
  threshold                 = 60
  alarm_description         = "Average connection count is too high, performance may be negatively impacted."
  alarm_actions             = var.actions
  ok_actions                = var.actions
  insufficient_data_actions = var.actions
  tags                      = var.default_tags
...
resource "aws_cloudwatch_metric_alarm" "connection_count_too_low" {
  alarm_name                = "${var.prefix}-rds-${var.rds_instance_id}-lowConnectionCount"
  comparison_operator       = "LessThanThreshold"
  evaluation_periods        = var.evaluation_period
  metric_name               = "DatabaseConnections"
  namespace                 = "AWS/RDS"
  ...
  statistic                 = "Average"
  threshold                 = 30
  alarm_description         = "Average connection count is too low, there may be an issue somewhere preventing clients from connecting."
  alarm_actions             = var.actions
  ok_actions                = var.actions
  insufficient_data_actions = var.actions
  tags                      = var.default_tags
...
resource "aws_cloudwatch_metric_alarm" "connection_count_anomalous" {
  alarm_name                = "${var.prefix}-rds-${var.rds_instance_id}-anomalousConnectionCount"
